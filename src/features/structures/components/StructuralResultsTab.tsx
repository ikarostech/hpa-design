...
import { StructuralPropertiesCharts } from "./StructuralPropertiesCharts";
...
export function StructuralResultsTab({ results, result, onSelectResult }: {
  ...
  return <div className="space-y-5">
    ...
    <Card>
      <CardHeader><h2 className="font-semibold">パイプ特性一覧</h2></CardHeader>
      <CardBody><div className="max-h-96 overflow-auto"><table className="w-full min-w-[900px] text-left text-sm">
        <thead className="sticky top-0 bg-white text-xs text-slate-500"><tr><th className="px-2 py-2">Y</th><th>外径</th><th>板厚</th><th>曲げ剛性 EI</th><th>ねじり剛性 GJ</th><th>曲げ耐荷重</th><th>ねじり耐荷重</th><th>線密度</th></tr></thead>
        <tbody>{sampledPoints.map((point) => <tr key={point.yPosition} className="border-t"><td className="px-2 py-2">{point.yPosition.toFixed(3)} m</td><td>{(point.outerDiameter * 1000).toFixed(1)} mm</td><td>{(point.thickness * 1000).toFixed(2)} mm</td><td>{point.ei.toExponential(3)} Nm²</td><td>{point.gj.toExponential(3)} Nm²</td><td>{formatOptional(point.bendingMomentCapacity, "Nm")}</td><td>{formatOptional(point.torqueCapacity, "Nm")}</td><td>{point.linearMass.toFixed(3)} kg/m</td></tr>)}</tbody>
      </table></div></CardBody>
    </Card>
    <Card>
